...
os.chdir(os.path.join(pkg_resources.get_distribution("spamEr").location, "spamEr"))
...
def update(force: bool = False):
    output = subprocess.run(
        ["pip3", "install", "spamEr", "--upgrade"],
        stdout=subprocess.PIPE,
        stderr=subprocess.DEVNULL,
    )
    if force or "Successfully installed spamEr" in output.stdout.decode():
        subprocess.run(
            ["pip3", "install", "spamEr", "--force-reinstall" if force else "--upgrade"],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        os.execlp("spamEr", " ".join(sys.argv[1:]) or "--port 8080")
